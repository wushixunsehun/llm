# -*- encoding: UTF-8 -*-
'''
@File  : 30Bqwq_stable.py
@Env   : Python 3.11.11
@Desc  : conda env llm=0.9.4
@Time  : 2025-07-28 16:33:20
@Author: tanxh
'''
...
async def get_models():
    return JSONResponse({
        ...
        "data": [
            {
                "id": cfg["model"].split("/")[-1],  # 这个必须和 chat 请求中的 model 保持一致
                "object": "model",
...
async def chat(request: Request):
    ...
    sampling = SamplingParams(
        ...
        top_p=data.get("top_p", cfg["top_p"]),
        top_k=data.get("top_k", cfg["top_k"]),
        max_tokens=data.get("max_tokens", cfg["max_tokens"]),
        presence_penalty=data.get("presence_penalty", cfg["presence_penalty"]),
    )
